...
class AxiStream
    ...
    def axis_slaver_pipe(axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
        ...
        axis_out = self unless self==AxiStream.NC
         ...
         @instance_draw_stack << lambda { axis_slaver_pipe_draw(axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream) }
        return return_stream
    end

    def axis_slaver_pipe_draw(axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)

        large_name_len(axis_in,axis_out)
"
// FilePath:::../../axi/AXI_stream/axis_slaver_pipe.sv
axis_slaver_pipe axis_slaver_pipe_#{signal}_inst(
...
"
    ...
    def self.axis_slaver_pipe(axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
        return_stream = nil
        ...
        if down_stream.is_a? AxiStream
            down_stream.axis_slaver_pipe(axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        elsif axis_out.is_a? AxiStream
            axis_out.axis_slaver_pipe(axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        else
            AxiStream.NC.axis_slaver_pipe(axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        end
        return return_stream
    end
...
class TdlTest
    ...
    def self.test_axis_slaver_pipe
        c0 = Clock.new(name:"axis_slaver_pipe_clk",freqM:148.5)
        r0 = Reset.new(name:"axis_slaver_pipe_rst_n",active:"low")
        ...
        AxiStream.axis_slaver_pipe(axis_in:axis_in,axis_out:axis_out)
