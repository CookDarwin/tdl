...
class AxiStream
    ...
    def axis_valve(button:"button",axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
        ...
        axis_out = self unless self==AxiStream.NC
         ...
         @instance_draw_stack << lambda { axis_valve_draw(button:button,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream) }
        return return_stream
    end

    def axis_valve_draw(button:"button",axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)

        large_name_len(button,axis_in,axis_out)
"
// FilePath:::../../axi/AXI_stream/axis_valve.sv
axis_valve axis_valve_#{signal}_inst(
...
"
    ...
    def self.axis_valve(button:"button",axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
        return_stream = nil
        ...
        if down_stream.is_a? AxiStream
            down_stream.axis_valve(button:button,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        elsif axis_out.is_a? AxiStream
            axis_out.axis_valve(button:button,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        else
            AxiStream.NC.axis_valve(button:button,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        end
        return return_stream
    end
...
class TdlTest
    ...
    def self.test_axis_valve
        c0 = Clock.new(name:"axis_valve_clk",freqM:148.5)
        r0 = Reset.new(name:"axis_valve_rst_n",active:"low")
        ...
        AxiStream.axis_valve(button:button,axis_in:axis_in,axis_out:axis_out)
