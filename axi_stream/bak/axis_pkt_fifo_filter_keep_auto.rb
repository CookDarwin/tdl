...
class AxiStream
    ...
    def axis_pkt_fifo_filter_keep(depth:2   ,axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
        ...
        axis_out = self unless self==AxiStream.NC
         ...
         @instance_draw_stack << lambda { axis_pkt_fifo_filter_keep_draw(depth:depth,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream) }
        return return_stream
    end

    def axis_pkt_fifo_filter_keep_draw(depth:2   ,axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
...
) axis_pkt_fifo_filter_keep_#{signal}_inst(
...
"
    ...
    def self.axis_pkt_fifo_filter_keep(depth:2   ,axis_in:"axis_in",axis_out:"axis_out",up_stream:nil,down_stream:nil)
        return_stream = nil
        ...
        if down_stream.is_a? AxiStream
            down_stream.axis_pkt_fifo_filter_keep(depth:depth,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        elsif axis_out.is_a? AxiStream
            axis_out.axis_pkt_fifo_filter_keep(depth:depth,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        else
            AxiStream.NC.axis_pkt_fifo_filter_keep(depth:depth,axis_in:axis_in,axis_out:axis_out,up_stream:up_stream,down_stream:down_stream)
        end
        return return_stream
    end
...
class TdlTest
    ...
    def self.test_axis_pkt_fifo_filter_keep
        c0 = Clock.new(name:"axis_pkt_fifo_filter_keep_clk",freqM:148.5)
        r0 = Reset.new(name:"axis_pkt_fifo_filter_keep_rst_n",active:"low")
        ...
        AxiStream.axis_pkt_fifo_filter_keep(depth:depth,axis_in:axis_in,axis_out:axis_out)
